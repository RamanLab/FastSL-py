...
def _double_lethal_reactions_phase_1_helper(model, cutoff, grWT, delIdx_i, delIdx_j):
    with model:
...
def _double_lethal_reactions_phase_1(model, cutoff, grWT, Jnz, eliIdx, delIdx_i):
    with model:
        model.reactions[delIdx_i].knock_out()
...
def doubleSL(model, cutoff, eliList, solver):
    ...
    # Jdl_idx = Jdl_idx_1 + Jdl_idx_2
    Jdl = Jdl_idx_1
    Jdl = [j for i in list(filter(None, Jdl)) for j in i]
